...
/** ★출력형태 공통 법적 규칙 — 블로그·슬라이드 모두에 붙는다.
 *  문체(blogModeRules)와 분리해 둔 이유: 문체는 형태별로 달라도 법적 요구는 같기 때문이다.
 *  섞여 있으면 형태 분기에서 법적 규칙이 함께 빠지는 사고가 난다(실제로 슬라이드에서 그랬다). */
const LEGAL_RULES = `

[⚠️법적 필수 — 출력형태와 무관하게 항상 적용]
- ⭐후기/리뷰 섹션 — 전 카테고리 공통(표시광고법 / 공정위, 가짜 후기는 과징금 대상):
   · 셀러가 실제 후기를 입력했으면 → 그 후기를 그대로 표시(작성자·별점도 입력값이 있을 때만).
   · 셀러가 후기를 입력하지 않았으면 → 1인칭 과거형 경험담·별점·작성자명을 절대 생성하지 마세요.
     금지 예시 → "샀는데 좋았어요" / "저는 써봤는데요" / "큰 기대 안 했는데" / "우리 아이가 좋아해요" / "★★★★★" / 작성자명.
   · 대신 미래형 기대 시나리오로: "이런 분들이 만족하실 거예요" — 단정·보장이 아니라 기대형으로.
   · quote 블록(작성자/별점) 금지, rating 금지. 단 후기 섹션 자체는 유지(셀러가 진짜 후기를 넣을 자리).
- ⛔셀러에게 말을 거는 안내문을 카피에 절대 넣지 마세요 — "실제 후기를 입력하면 이 섹션이 강력해집니다" 같은
  문구는 셀러용 안내인데 카피에 쓰면 고객이 보는 상세페이지·다운로드 파일에 그대로 실립니다.
  카피에는 고객이 읽을 말만 쓰세요.
- ⛔셀러가 입력하지 않은 수치·성분·인증·수상·임상·용량·가격을 만들지 마세요. 입력값을 그대로 쓰되,
  없으면 그 주제를 아예 다루지 않는 편이 낫습니다.`;
...
특히 아래 넷은 '그럴듯해서' 자주 나오는 날조입니다. 절대 쓰지 마세요:
 ...
 5. 제3자의 추천·보증 창작 — "스타일리스트들이 말하는", "전문가들이 추천하는", "○○에서 인정받은"
    → 공정위 추천·보증 심사지침 위반입니다. 위 재료에 실명 추천이 없으면 추천 주체를 만들지 마세요.
      원리·상식을 설명하고 싶으면 주체 없이 쓰세요("어깨선이 내려오면 팔 라인이 덜 드러납니다").
 6. 교환·반품·A/S·배송 조건 창작 — "미착용이면 교환돼요", "무료 반품", "○일 이내 환불", "보증 기간"
    → 전자상거래법상 셀러가 실제로 정한 조건만 표시할 수 있습니다. 위 재료에 없으면 조건을 쓰지 말고,
      그 주제 자체를 다루지 마세요(고객이 이 문장을 근거로 청약철회를 요구하면 셀러가 책임집니다).
 7. 유통·원가 구조 창작 — "중간 유통을 줄여서", "직거래라 저렴하게", "거품을 뺐어요"
    → 위 재료에 유통 구조가 적혀 있지 않으면 가격이 싼 이유를 지어내지 마세요.

⛔섹션 이름이 재료를 만들어내지 않습니다.
  '전문가 추천'·'생산 과정'·'A/S 보증' 같은 이름이 붙어 있어도, 그 주제의 재료가 위에 없으면
  이름에 맞춰 내용을 지어내지 마세요. 있는 재료로 쓸 수 있는 가장 가까운 내용을 쓰거나, 짧게 끝내세요.
  섹션을 채우는 것보다 없는 사실을 넣지 않는 것이 우선입니다.
...
8. ★강조 마킹(2026-07-27 — 렌더가 볼드·포인트 컬러로 표시): body와 subcopy에서만 사용.
   ...
   · ((문장)) = 포인트 컬러 강조 — ★대략 5섹션에 1곳꼴입니다(섹션당 1곳이 아닙니다).
     '어' 하고 멈추게 하는 결정적 한 줄에만 씌우세요. 매 섹션 끝을 색칠하면 장식이 되고,
     페이지에 하나뿐이면 스크롤 내내 색이 없어 밋밋합니다(둘 다 실측으로 확인).
     최종 개수는 렌더러가 페이지당 2~6곳으로 정돈하니 과감하게 후보를 내되 남발하지 마세요.
...
- ★★어투(speech_level) = ${ss.speech_level || '해요체'} — 이 지시는 아래의 문체(writing_style)·화자·대화체 v5 지시보다 우선합니다.
  실측(2026-08-04): '대화체로 쓰라'는 지시가 어투를 이겨 명사형을 골라도 해요체가 나왔습니다.
  충돌하면 어투가 이깁니다 — 명사형이면 문장을 체언으로 끝내고("3년을 매달린 이유"),
  단정형이면 ~다로, 합니다체면 ~합니다로. 전 섹션 통일, 중간에 섞지 마세요.
   · 해요체 → "~해요/~어요/~죠" 친근한 존댓말.
...
`;
  ...
  /* ★법적 규칙은 출력형태와 무관하게 항상 붙인다(2026-08-02 수정).
   *  이전엔 이 규칙들이 blogModeRules(블로그 문체 뭉치) 안에 섞여 있어서, 슬라이드를 제외할 때
   *  '가짜 후기 금지' 같은 법적 필수까지 통째로 빠졌다. 문체는 형태별로 달라도 법은 같다.
   *  (사후 그물 factScrub은 양쪽에 걸려 있었지만, 그건 수치·후기 위주라 예방 지시를 대체하지 못한다) */
  const composedSystem = isSlide
    ? system + LEGAL_RULES
    : system + blogModeRules + LEGAL_RULES;
...
⛔stats·iconcards에 가격·할인율·정가·판매가를 넣지 마세요("24,000원"·"20% 할인" 카드 금지).
  가격은 마지막 구매 정보 스트립이 셀러 입력값으로 따로 보여줍니다 — 카피 블록에 겹치면
  값이 어긋날 때 셀러가 고칠 곳이 두 군데가 됩니다. 블록 안에서도 copyGuards를 동일하게 지키세요(미입력 사실·과장 금지, 효능은 헤지). 실제 후기가 입력되지 않았다면 단정적 후기를 지어내지 말고 quote 블록은 생략하고 body에서 기대 시나리오로 처리하세요.
...
- 단, 반복 금지는 '표현과 서술'의 규칙입니다 — strategy_summary의 전략·톤은 전 섹션이 공유합니다.`
  ...
  const noMarkBlock = salesMode && !isSlide && !killerBlock
    ? '\n[포인트 컬러] 이 청크에서는 결정적 한 줄이 있는 섹션에만 ((강조))를 쓰세요 — 청크당 최대 1~2곳.\n(킬러 라인은 다른 섹션이 맡고 있으니 그보다 약한 문장에 씌우지 마세요. 최종 개수는 렌더러가 페이지당 2~6곳으로 정돈합니다.) **볼드**는 셀러 입력 팩트에 한해 섹션당 1곳까지 허용합니다.'
    : '';
  ...
  /* ★날조 차단은 슬라이드에도 붙인다(2026-08-02) — 형태와 무관하게 '없는 사실을 만들지 않는다'는
   *  같은 요구다. 훅·킬러라인·분량 차등은 블로그 문체 전제라 블로그에만 남긴다.
   *  (어투(speech_level)도 슬라이드에서는 제외 — 카피가 1~2문장이라 체감이 거의 없다는 유근님 판단) */
  const antiFabBlock = salesMode ? antiFabricationRule(knownFacts ?? '') : '';
...
⚠️'~에의'·'~으로의'·'~에서의' 같은 번역투 조사를 만들지 마라 — 명사형이라도 자연스러운 한국어로
끝낸다("서늘한 곳에의 보관" ✗ → "서늘한 곳 보관" ✓, "제품에의 실망" ✗ → "제품에 남은 실망" ✓). 입력은 ⟨§⟩ 구분자로 나뉜 ${items.length}개 항목이다.
