...
/**
 * Stage3 (카피 생성, v5) — 검증된 프로토타입 로직을 lib으로 추출(통합 파이프라인 공유용).
 * app/api/copy/route.ts 의 핵심 로직(v5: emotion_goal 우선 + writing_style + Character + 블록)을
 * 그대로 옮긴 것. 동작 변경 없음.
 *
 * 청크 분할(CHUNK_SIZE=16)은 프로토타입 검증 단계에 이미 들어있던 동작이라 그대로 유지한다.
 * (통합 3단계의 "청크분할 주입"은 별개의 가드/과금방지 정책 레이어를 가리키며 여기서 다루지 않음)
 */
...
- ⭐바닥 톤(brand tone)은 모든 섹션이 공유합니다: ${strategy.tone || '신뢰감 있되 친근한'}. 이 톤은 16개 섹션 전체에 깔리는 한 사람의 목소리입니다.
...
- ⭐통일감: 화자가 바뀌어도 브랜드 기본 톤(${strategy.tone || '신뢰감 있되 친근한'})은 전 섹션 공통입니다. 16명이 따로 쓴 문서가 아니라, 하나의 브랜드가 상황에 맞는 화자를 내세우는 느낌이어야 합니다. 호칭·존댓말 수준·독자를 부르는 방식은 일관되게. 과도한 화자 분리·산만함은 금지.
...
${dna?.target_desire ? `- ★고객이 진짜 원하는 것(target_desire) — 페이지 전체를 관통하는 감정선, 섹션마다 다른 장면으로 변주: ${dna.target_desire}` : ''}
${dna?.target_fear ? `- 고객이 가장 두려워하는 것(target_fear) — 초반 설득 + FAQ/비교에서 해소 대상: ${dna.target_fear}` : ''}
...
[
  {
    ...
    if (message.stop_reason === 'max_tokens') {
      throw new Error(`청크(${startIdx}~)가 max_tokens(16000)에 도달해 잘렸어요. 청크 크기를 줄여보세요.`);
    }
