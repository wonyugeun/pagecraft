...
- ★show/format은 상품정보에 실제로 존재하는 사실·데이터만 보여주라고 지시할 수 있습니다. 다음이 상품정보에 없다면 그것을 보여주라는 지시 자체를 만들지 마세요(지시하면 이미지가 없는 데이터를 지어냅니다): 별점·리뷰 수·판매량·재구매율·만족도·순위 / 크기·중량·함량 수치 / 낱개·타사 구매 비용 비교 / 배송·교환·반품 정책 / 성분·원료의 개별 이름('12종'처럼 개수만 제공됐다면 개수까지만). '별점 지표'·'경쟁 비교'·'가격 앵커링' 같은 섹션명이 오더라도 상품정보에 있는 사실만으로 재해석하세요.
- ★고객 후기는 상품정보에 인용된 원문·개수까지만 쓸 수 있습니다. 후기 섹션이 여러 개면 제공된 후기를 나눠 배치하거나, 후기 인용이 없는 방식(사용 장면·타이포)으로 재해석하세요 — 새 후기·새 작성자명을 만들라는 지시는 금지입니다.
- 이미지 안에 버튼·클릭 유도 UI(지금 시작하기/구매하기 등)를 넣으라고 지시하지 마세요 — 실제 구매 버튼은 플랫폼이 제공합니다.
- 다른 브랜드·라벨이 있는 용기는 절대 연출하지 마세요. 문제 상황 묘사에 필요하면 라벨 없는 무지 용기만 허용되고, 그 무지 용기는 우리 제품과 실루엣·뚜껑·내용물 색이 확연히 달라야 합니다(우리 제품이 자극 원인처럼 보이면 안 됩니다).
