...
export interface SectionPlan {
  name?: string;
  /** ★셀러가 섹션구조 화면에서 읽을 한 줄 설명(2026-08-03) — 이름만으로는 '병풀 성분 심층'이
   *  무엇을 담는 섹션인지 알 수 없다. 용어집은 AI가 짓는 이름을 따라갈 수 없어 여기서 받는다. */
  desc?: string;
  role?: string;
  mission?: string;
  emotion_goal?: string;
...
설명이 없는 섹션은 이름과 전략만 보고 자유롭게 설계하세요(셀러가 직접 적어 넣은 섹션입니다).\n` : ''}`
   ...
   *  섹션 이름이 카피의 방향을 정하는데, 이름 단계가 무방비면 "트러블 진정 효과" 같은 제목이 나오고
   *  카피라이터는 금지어만 피한 채 그 방향으로 쓴다. 뒤에서 거르는 것보다 이름을 안 짓게 하는 편이 낫다.
   *  가드는 카피와 같은 것을 쓴다(lib/copyGuards) — 두 벌로 나뉘면 반드시 어긋난다. */
  const legalGuard = getCategoryCopyGuard(cat || '');
...
- ⚠️섹션 이름은 '효과 단정'이 아니라 '다룰 주제'로 지으세요. 이름이 카피의 방향을 정하기 때문입니다.
  카피 단계에 법규 가드가 걸려 있어도, 이름이 효능을 단정해 버리면 카피라이터가 그 방향으로 쓰려 합니다.
    ✗ "트러블 진정 효과" · "주름 개선 효과" · "면역력 증진"   (효과가 난다고 단정)
    ✓ "진정 성분 이야기" · "탄력 관리 루틴" · "섭취 가이드"     (무엇을 다루는지)
- ⚠️셀러가 주지 않은 재료를 요구하는 섹션 이름을 만들지 마세요 — 그 이름을 받으면 카피가 지어냅니다.
    ✗ "전문가 추천 코멘트"(추천한 전문가가 없을 때) · "별점·리뷰 요약"(별점이 없을 때)
    ✗ "임상 시험 결과"(시험 자료가 없을 때) · "수상 이력"(수상이 없을 때)
- ⚠️각 섹션에 desc를 한 줄 쓰세요 — 셀러가 섹션구조 화면에서 "이게 뭔지" 판단하는 유일한 근거입니다.
  이름만으로는 '병풀 성분 심층'이 무엇을 담는 섹션인지 셀러가 알 수 없습니다.
...
[출력 형식 — 아래 JSON 객체 하나만. 다른 텍스트·설명·마크다운·코드펜스 금지]
{
  ...
  "sections": [
    { "name": "섹션 이름(12자 이내 역할명)", "desc": "셀러가 읽을 한 줄 설명 — 이 섹션에 무엇이 담기는지(예: \"병풀·판테놀이 왜 들어갔는지 근거와 함께 보여줘요\")", "role": "이 섹션의 역할", "mission": "이 제품의 전략/main_weapon을 어떤 각도로 수행하는지 한 줄", "emotion_goal": "이 섹션을 읽고 독자가 속으로 느껴야 할 한 문장 (독자의 1인칭 속마음)", "writing_style": "이 섹션을 쓸 말하는 방식 한 줄 (예: 친구가 말하듯 질문·대화체 / 선언형 짧고 강한 문장 / 계산형 숫자 중심 등)", "block_plan": "'글만' 또는 블록 타입 1~2개 + 선택 이유 한 마디 (예: 'compare — 일반 품종과 식감 차이가 설득 핵심')" }
  ]
}`;
