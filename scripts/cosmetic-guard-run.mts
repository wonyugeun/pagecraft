/**
 * 화장품 — 구조 단계 법규 가드 검증용(2026-08-03).
 *
 * ★확인할 것: 섹션 이름이 '효과 단정'에서 '주제'로 바뀌는지.
 *   전(가드 없음): 트러블 진정 효과 · 수분 공급 효과 · 전문가 추천 코멘트 · 별점·리뷰 요약
 *   기대(가드 있음): 진정 성분 이야기 · 보습 사용감 · (재료 없는 섹션은 아예 안 생김)
 * ★그리고 섹션마다 desc(셀러용 한 줄 설명)가 나오는지 — 32섹션 중 16개가 설명이 없었다.
 *
...
const REF_PATH = path.join(ROOT, 'test-assets', 'leafgreen.png');
...
const PRESET = {
  cat: '화장품', ch: '스마트스토어',
  productName: 'LEAFGREEN 시카 카밍 토너 250ml',
  productExtra: [
    '[화장품 종류]: 스킨/토너',
    '[주요 피부 고민]: 민감함, 붉어짐, 건조함',
    '[핵심 성분]: 병풀추출물(센텔라아시아티카), 판테놀, 히알루론산',
    '[사용 방법]: 세안 후 화장솜에 적셔 얼굴 전체에 부드럽게 닦아내듯 사용',
    '[용량]: 250ml',
    '[인증/특징]: 무알콜, 무향료, 저자극 테스트 완료',
    '[제조]: 국내 제조',
    '차별점: 자극적인 각질 제거 없이 닦아내는 토너 — 민감한 날에도 매일 쓸 수 있게 알코올과 향료를 뺐어요',
    '가격: 정가 28000원 / 할인가 19800원',
    '기타 요청사항: 제형은 맑은 무색 액체입니다. 걸쭉하거나 색이 있는 제형으로 그리지 말 것.',
  ].join('\n'),
  productVolume: '250ml',
};
...
const SECTION_TARGET = 12;   // ★이름을 AI가 짓게 둔다 — 가드가 이름에 먹히는지가 검증 대상
const SECTION_COUNT = SECTION_TARGET;
...
async function main() {
  ...
  const outDir = path.join(ROOT, 'runs', `화장품-구조가드-${stamp}`);
  fs.mkdirSync(outDir, { recursive: true });

  console.log(`[run] 화장품 구조 가드 ${SECTION_TARGET}섹션 — 카피 파이프라인 시작 (${BASE_URL})`);
  const t0 = Date.now();
  const job = createJob({
