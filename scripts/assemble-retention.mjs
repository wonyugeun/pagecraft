...
<body><div class="wrap">
  <div class="banner">
    <h1>${esc(productName)} — 전략 유지율 검증 (Stage1 고정)</h1>
    <p>고정 Stage1 → structure → copy(v3) · 브랜드 톤: ${esc(strat.tone || '')} · ${esc(cat)}/${esc(ch)}/${esc(copy.meta?.form || '블로그형')}/${esc(depth)}${secs.length}섹션 · 이미지 미생성</p>
  </div>
...
(async () => {
  console.log(`▶ 전략 유지율 검증  BASE=${BASE}  cat=${cat} ch=${ch} depth=${depth}\n`);

  console.log('[1/3] strategy (고정)…');
  ...
  console.log(`   sections=${structure.sections?.length}  writing_style=${withStyle}`);

  console.log('\n[3/3] copy (고정 Stage1 + 구조 투입, v3 그대로)…');
  ...
  const htmlPath = join(OUT_DIR, 'retention-test.html');
  const jsonPath = join(OUT_DIR, 'retention-test.json');
