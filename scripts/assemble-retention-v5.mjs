...
<body><div class="wrap">
  <div class="banner">
    <h1>${esc(productName)} — 전략 유지율 검증 v5 (Stage1·Stage2 고정, Stage3만 v5)</h1>
    <p>고정 Stage1 + 동일 구조 → copy(v5: 섹션별 Character 화자) · 브랜드 톤: ${esc(strat.tone || '')} · ${esc(cat)}/${esc(ch)}/${esc(copy.meta?.form || '블로그형')}/${esc(depth)}${secs.length}섹션 · 이미지 미생성</p>
  </div>
...
(async () => {
  console.log(`▶ 전략 유지율 검증 v5  BASE=${BASE}  cat=${cat} ch=${ch} depth=${depth}\n`);
  ...
  // Stage2 재사용 — v3 retention 실행의 구조(retention-test.json)를 그대로 써서 Stage3만 v5로 비교.
  ...
  console.log('\n[3/3] copy (고정 Stage1 + 동일 구조 투입, v5 프롬프트)…');
  ...
  const htmlPath = join(OUT_DIR, 'retention-test-v5.html');
  const jsonPath = join(OUT_DIR, 'retention-test-v5.json');
