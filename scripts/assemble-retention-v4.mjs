/**
 * Stage1→2→3 전략 유지율 검증 (강아지 관절영양제, 화장품 정답지 비대상 카테고리).
 * Stage1(strategy)은 1회만 호출해 retention-stage1.json으로 고정 — 이후 재생성 금지(파일 있으면 재사용).
 * 고정한 Stage1 → Stage2(structure) → Stage3(copy, 현재 v3 상태 그대로) 순차 실행.
 *
 * 사용: dev 서버 띄운 채  node scripts/assemble-retention.mjs
 * 출력: _prototype_out/retention-test.html + retention-test.json (+ 고정 retention-stage1.json)
 */
...
const STAGE1_FIXED = join(OUT_DIR, 'retention-stage1.json'); // 고정 Stage1 캐시

const cat = '반려동물';
const ch = '스마트스토어';
const depth = '간결';
const sectionCount = null; // 반려동물 기준 섹션수(간결=14)를 structure가 산출하도록 강제하지 않음
const productName = '튼튼발 노령견 관절 츄어블';

const productExtra = [
  '브랜드명: 튼튼발(가칭)',
  '[주요 성분]: 글루코사민, MSM, 녹색입홍합',
  '경쟁 차별점: 노령견 관절 케어, 계단 오르기 힘들어하는 노령견용, 기호성 좋은 츄어블 형태',
  '기타 요청사항: 7세 이상 노령견 보호자 타겟, 산책을 좋아하던 아이가 느려진 게 속상한 보호자, 따뜻하고 공감되는 톤',
].join('\n');
...
<body><div class="wrap">
  <div class="banner">
    <h1>${esc(productName)} — 전략 유지율 검증 v4 (Stage1·Stage2 고정, Stage3만 v4)</h1>
    <p>고정 Stage1 + 동일 구조 → copy(v4: emotion 우선·target_desire 관통) · 브랜드 톤: ${esc(strat.tone || '')} · ${esc(cat)}/${esc(ch)}/${esc(copy.meta?.form || '블로그형')}/${esc(depth)}${secs.length}섹션 · 이미지 미생성</p>
  </div>
  ...
  <div class="foot">전략 유지율 검증 · Stage1 고정값 → Stage2 → Stage3(v3) · 기존 generate 미변경 · ${new Date().toISOString()}</div>
</div></body></html>`;
}

// Stage1 고정: 캐시 파일이 있으면 재사용(재생성 금지), 없으면 1회 호출 후 저장.
async function getFixedStage1() {
  if (existsSync(STAGE1_FIXED)) {
    const fixed = JSON.parse(readFileSync(STAGE1_FIXED, 'utf-8'));
    console.log(`🔒 Stage1 고정값 재사용: ${STAGE1_FIXED} (재생성 안 함)`);
    return fixed;
  }
  // strategy는 max_tokens(1500)에 가끔 걸리므로 최대 4회 재시도 — 단 성공한 1개를 고정.
  let lastErr;
  for (let attempt = 1; attempt <= 4; attempt++) {
    try {
      const r = await call('/api/strategy', { cat, ch, productName, productExtra });
      writeFileSync(STAGE1_FIXED, JSON.stringify(r, null, 2), 'utf-8');
      console.log(`🔒 Stage1 1회 생성 후 고정 저장: ${STAGE1_FIXED}`);
      return r;
    } catch (e) { lastErr = e; console.log(`… strategy 시도 ${attempt} 실패: ${e.message}`); }
  }
  throw lastErr;
}

(async () => {
  console.log(`▶ 전략 유지율 검증 v4  BASE=${BASE}  cat=${cat} ch=${ch} depth=${depth}\n`);

  console.log('[1/3] strategy (고정 재사용)…');
  const strategy = await getFixedStage1();
  console.log('   main_weapon  :', strategy.dna?.main_weapon);
  console.log('   target_desire:', strategy.dna?.target_desire);

  // Stage2 재사용 — v3 retention 실행의 구조(retention-test.json)를 그대로 써서 Stage3만 v4로 비교.
  console.log('\n[2/3] structure (v3 retention 구조 재사용 — Stage3 변화만 격리)…');
  const V3 = JSON.parse(readFileSync(join(OUT_DIR, 'retention-test.json'), 'utf-8'));
  const structure = V3.structure;
  console.log(`   재사용 sections=${structure.sections?.length} (from retention-test.json)`);

  console.log('\n[3/3] copy (고정 Stage1 + 동일 구조 투입, v4 프롬프트)…');
  ...
  console.log(`   copy sections=${copy.sections?.length}  블록 포함=${withBlocks}/${copy.sections?.length}`, blockTypeCount);

  const html = buildHtml({ strategy, structure, copy });
  const htmlPath = join(OUT_DIR, 'retention-test-v4.html');
  const jsonPath = join(OUT_DIR, 'retention-test-v4.json');
