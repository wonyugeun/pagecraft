/**
 * 판정용 1회 조립 (v2) — Stage3 카피 개선("설명문→독자가 느끼는 카피" + 블록) 검증.
 * strategy → structure(emotion_goal 추가) → copy(emotion_goal 달성 + blocks) 순차 호출.
 * 이미지브리프·Gemini 제외(무과금). 기존 generate 파이프라인 미변경. 조립만.
 *
 * 사용: dev 서버 띄운 채  node scripts/assemble-onepage-v2.mjs
 * 출력: _prototype_out/onepage-judge-v2.html + onepage-judge-v2.json
 */
...
function buildHtml({ strategy, structure, copy }) {
  ...
  const planRows = plan.map((s, i) => `<tr><td>${i + 1}</td><td><b>${esc(s.name)}</b></td><td>${esc(s.mission)}</td><td class="emo">${esc(s.emotion_goal)}</td></tr>`).join('');
...
<body><div class="wrap">
  <div class="banner">
    <h1>리프그린 시카 토너 — 새 엔진 v2 (대화체 + 블록)</h1>
    <p>strategy → structure(+emotion_goal) → copy(emotion_goal 달성 + blocks) · ${esc(cat)}/${esc(ch)}/${esc(copy.meta?.form || '블로그형')}/${esc(depth)}${secs.length}섹션 · 이미지 미생성</p>
  </div>
  ...
  <div class="meta-card">
    <h3>Stage2 · 구조 + emotion_goal (신규)</h3>
    <table class="plan"><tr><th></th><th>섹션</th><th>mission</th><th>emotion_goal</th></tr>${planRows}</table>
  </div>

  <div class="meta-card" style="background:#f0fdf9;">
    <h3>Stage3 · 대화체 카피 + 블록 (${secs.length}섹션)</h3>
    <p style="margin:0;font-size:13px;color:#777;">각 섹션 상단 🎯 = 달성해야 할 emotion_goal. 본문 아래 회색/민트 박스 = 블록(checklist·iconcards·steps·compare·quote 등).</p>
  </div>
  <div class="page">${sections}</div>

  <div class="foot">판정용 1회 생성 (v2) · 기존 generate 파이프라인 미변경 · ${new Date().toISOString()}</div>
</div></body></html>`;
}
...
(async () => {
  ...
  console.log(`   sections=${structure.sections?.length}  emotion_goal 채워진 섹션=${withEmo}`);

  console.log('\n[3/3] copy (emotion_goal 달성 + blocks)…');
  ...
  const htmlPath = join(OUT_DIR, 'onepage-judge-v2.html');
  const jsonPath = join(OUT_DIR, 'onepage-judge-v2.json');
