 ...
 * ★환불 규정(2026-07-30 확정): 7일 이내 + 충전분 미사용 + 결제 이후 다운로드 없음 → 전액 환불.
 *   조건은 /api/admin/refund의 evaluateRefund가 데이터로 그대로 판정한다(문구와 동작 일치).
 *   ⚠️'할인가 구매 시 환불 불가'는 채택하지 않았다 — 할인이라는 이유만으로 청약철회를 배제하는
 *   조항은 전자상거래법상 무효 소지가 있고, 현재 전 플랜이 특가라 사실상 전건 환불 불가가 된다.
      ...
      <div style={{
        ...
        <Section title="환불 규정">
          <p style={{ ...bodyStyle, marginBottom: 14 }}>
            아래 조건을 <b>모두 충족</b>하면 결제 금액 전액을 환불해 드립니다.
          </p>
          <Bullets items={[
            ['결제 후 7일 이내', '결제일로부터 7일이 지나지 않았어야 합니다.'],
            ['충전한 크레딧 미사용', '충전분을 한 번도 사용하지 않았어야 합니다.'],
            ['다운로드 이력 없음', '결제 이후 결과물을 한 번도 내려받지 않았어야 합니다.'],
          ]} />
          <p style={{ ...bodyStyle, marginTop: 16 }}>
            위 조건 중 하나라도 해당하지 않으면, 디지털 콘텐츠의 특성상 환불이 제한됩니다
            (전자상거래법 제17조 제2항). 결제 전 신규 가입 체험 크레딧으로 서비스를 충분히 확인하실 수 있습니다.
          </p>
          <p style={{ ...bodyStyle, marginTop: 10 }}>
            환불은 <b>flik.support@gmail.com</b> 으로 신청해 주세요. 확인 후 영업일 기준 3일 이내에
            결제 대행사를 통해 처리되며, 카드사 사정에 따라 실제 입금까지 며칠이 더 걸릴 수 있습니다.
          </p>
          <p style={{ ...bodyStyle, marginTop: 10, fontSize: 14, color: '#8B95A1' }}>
            ※ 생성이 정상적으로 완료되지 않은 경우의 크레딧 자동 환불은 위 규정과 별개로 항상 적용됩니다.
          </p>
        </Section>
