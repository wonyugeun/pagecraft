...
/**
 * 썸네일 크레딧 선차감 게이트 — ★유일 차감·jobKey 발급 지점 (quick/charge와 동일 패턴).
 *
 * ThumbScreen이 생성(generate) 첫 동작으로 호출한다. deductCreditsAtomic(단일 CTE,
 * idempotency_key=jobKey UNIQUE)이 "차감 + generation:{count} 원장 기록"을 원자적으로 수행 →
 * 이후 generate-image는 그 jobKey로 verifyPaidJob만 통과(재차감 없음, verify-only).
 * - 비용: calculateGenerationCost(썸네일=1크레딧). insufficient면 402(외부 호출 0).
 * - 멱등: 같은 jobKey 재호출 = duplicate(추가 차감 0) → 실패/재시도 무료(C정책).
...
export async function POST(req: NextRequest) {
  ...
  if (!creditsBypassEnabled()) {
    ...
    try {
      ...
      console.error('[thumb/charge] 크레딧 차감 오류:', err);
