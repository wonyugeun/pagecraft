 ...
 *
 * Flik 엔진 재설계: 입력 → Stage1(전략) → Stage2(구조) → Stage3(카피) → Stage4(이미지 브리프).
 * 이 라우트는 Stage1~3 출력(strategy·sections·copy)을 받아 섹션별 "촬영 지시문"을 만든다.
 * 실제 Gemini 이미지 생성은 하지 않는다(브리프 JSON만).
 *
 * 검증 핵심:
 *  ① 브리프 mood/palette가 strategy.tone·concept을 따라가나
 *  ② 기존 이미지 규칙(인물 금지/제품 일관성/블로그형 텍스트 없는 사진/비율 9:16금지/구성품 폴백)이
 *     브리프에 반영되나
 *
 * 기존 generate/generate-image/imagePromptRules/sectionAspect/copyGuards는 읽기 전용.
 * ratio는 신뢰성을 위해 코드에서 aspectRatioFor로 결정(9:16 원천 차단), 나머지 필드만 모델이 채움.
 */
