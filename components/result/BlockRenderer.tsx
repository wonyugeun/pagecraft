...
function IconCardsBlock({ cards }: { cards: { title: string; desc?: string }[] }) {
  const cols = cards.length >= 4 ? 4 : Math.max(2, cards.length);
  return (
    <div style={{
      marginBottom: 32,
  ...
  return (
    <div style={{
      ...
      {items.map((s, i) => (
        <div key={i} style={{
          padding: 20, textAlign: 'center',
          borderRight: i !== items.length - 1 ? `1px solid ${COLORS.lightPurpleBorder}` : 'none',
        }}>
...
function CtaBlock({ text, button }: { text: string; button: string }) {
  return (
    <div style={{
      ...
      <button style={{
        marginTop: 24,
        display: 'inline-flex', height: 48, padding: '0 24px',
